...
primitive TestData
    ...
    fun _getInitializeParams() : (InitializeParams | None) =>
        try
            let json = JsonDoc
    ...
    fun getClientInfo() : (ClientInfo | None) =>
        try
            (TestData._getInitializeParams() as InitializeParams).clientInfo
        end

    fun getClientCapabilities() : (ClientCapabilities | None) =>
        try
            (TestData._getInitializeParams() as InitializeParams).capabilities
        end
